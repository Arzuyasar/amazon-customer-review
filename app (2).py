...
KURALLAR = {
    ...
    "Yazilim Ekibi": [
        ...
        "install","compatible","firmware","driver","bluetooth","wifi",
    ],
    "Lojistik": [
    ...
    "Musteri Hizmetleri": [
        ...
        "replied","contact","ignored","rude","exchange",
    ],
    "Urun Yonetimi": [
        "design","quality","cheap","material","size","color","uncomfortable",
        "usability","confusing","misleading","overpriced","flimsy",
    ],
}
...
def ozet_cikar(metin, cumle=2):
    """
    Basit extractive ozetleme — hic bagimlilik gerektirmez.
    En uzun ve en fazla kelime iceren cumleleri secer.
    """
    import re
    ...
    if len(cumleler) <= cumle:
        return metin
    
    # Her cumleye skor ver:
    # - Uzunluk (daha uzun = daha bilgi dolu)
    # - Problem anahtar kelimeleri iceriyor mu?
    problem_kelimeler = [
        "broken","not working","stopped","terrible","horrible","worst",
        "disappointed","useless","defective","failed","never","always",
        "problem","issue","error","crash","damage","refund","support"
    ]
    
    skorlar = []
    for c in cumleler:
        skor = len(c.split())  # kelime sayisi
        c_lower = c.lower()
        for k in problem_kelimeler:
            if k in c_lower:
                skor += 10  # problem kelimesi varsa bonus
        skorlar.append(skor)
    
    # En yuksek skorlu cumleleri al, orijinal sirayla diz
    en_iyi_idxler = sorted(
        sorted(range(len(skorlar)), key=lambda i: skorlar[i], reverse=True)[:cumle]
    )
    
    ozet = " ".join(cumleler[i] for i in en_iyi_idxler)
    return ozet
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='color:#6B7280;font-size:13px;margin-bottom:24px'>Amazon Review Analyzer</div>",
        unsafe_allow_html=True
    )
    sayfa = st.radio(
        "Sayfa",
...
if sayfa == "Tek Yorum Analizi":
    ...
    st.markdown(
        "<div style='color:#6B7280;margin-bottom:24px'>Yorum gir, ozet + departman + problem durumu al.</div>",
        unsafe_allow_html=True
    )
    ...
    with col2:
        ...
        yildiz_goster = "⭐" * star
        st.markdown(
            "<div style='text-align:center;font-size:28px'>" + yildiz_goster + "</div>",
            unsafe_allow_html=True
        )
    ...
    if btn and metin.strip():
        ...
        m1.markdown(
            ...
            "<div class='value' style='color:" + renk + ";font-size:20px'>" + durum + "</div>"
            "<div style='color:#6B7280;font-size:12px'>Skor: " + str(s["prob_skor"]) + "</div>"
            "</div>",
            unsafe_allow_html=True
        )
        ...
        m2.markdown(
            "<div class='metric-card'>"
            "<div class='label'>VADER</div>"
            "<div class='value' style='color:" + vr + "'>" + str(s["vader"]) + "</div>"
            "</div>",
            unsafe_allow_html=True
        )
        ...
        m3.markdown(
            ...
            "<div class='value'>%" + str(round(s["guven"] * 100)) + "</div>"
            "</div>",
            unsafe_allow_html=True
        )

        dr       = DEPT_RENK.get(s["departman"], "#ffffff")
        kutu_cls = "problem" if s["problem_var"] else "ok"
        st.markdown(
            "<div class='result-box " + kutu_cls + "'>"
            "<div style='font-size:12px;color:#6B7280;margin-bottom:8px'>DEPARTMAN</div>"
            "<span style='color:" + dr + ";font-size:18px;font-weight:600'>" + s["departman"] + "</span>"
            "</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            "<div style='font-size:12px;color:#6B7280;margin-bottom:8px'>OZET</div>"
            "<div class='summary-text'>" + s["ozet"] + "</div>"
            "</div>",
            unsafe_allow_html=True
        )
    ...
    if yuklenen:
        ...
        st.markdown("**" + str(len(df)) + " satir** yuklendi.")
        ...
        if st.button("Baslat", type="primary"):
            ...
            st.dataframe(
                res[["review_body", "departman", "problem_var", "guven", "vader", "ozet"]],
    ...
    if f:
        ...
        k1.metric("Toplam",    str(toplam))
        k2.metric("Problemli", str(prob))
        k3.metric("Oran",      "%" + str(round(prob / toplam * 100, 1)))
