...
def ozet_cikar(metin, cumle=2):
    """
    Abstractive ozetleme — facebook/bart-large-cnn modeli ile.
    Metni yeniden yazar, ilk cumleleri almaz.
    """
    ...
    try:
        from transformers import pipeline as hf_pipeline
        ozetleyici = hf_pipeline(
            "summarization",
            model="sshleifer/distilbart-cnn-12-6",
            device=-1,
        )
        # Model max 1024 token alir, metni kisalt
        kisaltilmis = metin[:1000]
        sonuc = ozetleyici(
            kisaltilmis,
            max_length=80,
            min_length=30,
        ...
        # Model yuklenemezse ilk 2 cumleyi don
        import re
        cumleler = re.split(r"(?<=[.!?])\s+", metin)
        cumleler = [c.strip() for c in cumleler if len(c.strip()) > 20]
        return " ".join(cumleler[:2]) if len(cumleler) >= 2 else metin[:200]
